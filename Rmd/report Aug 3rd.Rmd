...
## Look at CHIP + baseline labs à case_control - clogit
```{r, echo=TRUE}
...
## Take age out of models for myelosuppression-With Age
```{r, echo=TRUE}
logistic <- clogit(Cases_Controls ~ CHIP + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme+Prior_rad + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)
```

## Run models if only including VAF >10% as CHIP +
```{r, echo=TRUE}
logistic <- clogit(Cases_Controls ~ CHIP_VAF_10P + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_VAF_10P+BaseANC+BaseHGB+BasePLT+BaseWBC + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_VAF_10P+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_VAF_10P+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme+Prior_rad + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_VAF_10P+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)

```

## Case_Control ~ VAF + baseline labs + CHIP + strata
Use 10% cutoff for VAF -> Don't do
```{r}

```
## Look at Kelly’s paper and classify mutations by predicted response to chemo
DNA-damage response (DDR) genes: TP53, PPM1D, CHEK2
Choose best myelosuppression model and run for CHIPDD
```{r}
logistic <- clogit(Cases_Controls ~ CHIP_DDR + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_DDR+BaseANC+BaseHGB+BasePLT+BaseWBC + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_DDR+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_DDR+BaseANC+BaseHGB+BasePLT+BaseWBC+MAX2heme+Prior_rad + strata(Strata),
                   data = global_data)
summary(logistic)

logistic <- clogit(Cases_Controls ~ CHIP_DDR+MAX2heme + strata(Strata),
                   data = global_data)
summary(logistic)
```
